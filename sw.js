...
const CACHE = 'unranked-20260815145646';
...
self.addEventListener('fetch', e => {
  ...
  if (req.mode === 'navigate' || req.destination === 'document') {  // page: fresh when online, cached when offline
    e.respondWith(fetch(req).then(r => { if (r && r.ok) { const cp = r.clone(); caches.open(CACHE).then(c => c.put('./index.html', cp)); } return r; }).catch(() => caches.match('./index.html').then(r => r || caches.match('./'))));   // only cache a GOOD page — a 502/maintenance page must never overwrite the offline copy
    return;
  }
